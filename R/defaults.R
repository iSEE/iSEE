...
#'
#' It is also possible to store multiple alternative selections for point-based plots, and cycle between them as desired. 
#' These provide pre-constructed selections with which to replace the current \dQuote{active} selection in \code{BrushData} or \code{LassoData}.
#' \describe{
#' \item{\code{MultiSelectHistory}:}{A list of lists of brush or lasso objects containing stored alternative selections for each plot.
