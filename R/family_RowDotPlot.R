...
setMethod(".defineInterface", "RowDotPlot", function(x, se, select_info) {
    ...
    id <- x[[.organizationId]]
    list(
        .create_visual_box_for_row_plots(mode, id, x, select_info$single$row, select_info$single$column, se),
...
setMethod(".getCommandsDataSelect", "RowDotPlot", function(x, envir) {
    ...
    if (exists("row_selected", envir=envir, inherits=FALSE)) {
        ...
        cmds["SelectBy"] <- "plot.data$SelectBy <- rownames(plot.data) %in% row_selected;"
