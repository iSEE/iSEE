...
.create_width_height_observers <- function(x, input, org_pObjects) {
    ...
    width_name <- paste0(panel_name, "_", iSEEslots$organizationWidth)
    ...
    observeEvent(input[[width_name]], {
        ...
        cur.width <- slot(org_pObjects$memory[[panel_name]], iSEEslots$organizationWidth)
        new.width <- as.integer(input[[width_name]])
        if (!isTRUE(all.equal(new.width, cur.width))) {
            slot(org_pObjects$memory[[panel_name]], iSEEslots$organizationWidth) <- new.width
        }
    }, ignoreInit=TRUE)
    # nocov end

    height_name <- paste0(panel_name, "_", iSEEslots$organizationHeight)
    ...
    observeEvent(input[[height_name]], {
        ...
        cur.height <- slot(org_pObjects$memory[[panel_name]], iSEEslots$organizationHeight)
        new.height <- as.integer(input[[height_name]])
        if (!isTRUE(all.equal(new.height, cur.height))) {
            slot(org_pObjects$memory[[panel_name]], iSEEslots$organizationHeight) <- new.height
        }
    }, ignoreInit=TRUE)
    # nocov end
