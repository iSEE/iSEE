...
.make_featAssayPlot <- function(id, all_memory, all_coordinates, se, colormap) {
    ...
    data_cmds[["y"]] <- sprintf(
        "plot.data <- data.frame(Y=as.vector(assay(se, %i, withDimnames=FALSE)[%i,]), row.names = colnames(se))",
    ...
    if (x_choice==.featAssayXAxisColDataTitle) { # colData column selected
        ...
        data_cmds[["x"]] <- sprintf(
            "plot.data$X <- as.vector(assay(se, %i, withDimnames=FALSE)[%i,]);",
...
.make_sampAssayPlot <- function(id, all_memory, all_coordinates, se, colormap) {
    ...
    data_cmds[["y"]] <- sprintf(
        "plot.data <- data.frame(Y=as.vector(assay(se, %i, withDimnames=FALSE)[,%i]), row.names = rownames(se));",
