...
#' \itemize{
#' \item New values of the fields \code{by_field} and \code{tab_field} in \code{input} will be used to overwrite values in \code{pObjects$memory} for the current panel.
#' However, if \code{input[[by_field]]} is not equal to \code{title}, any linked table in the memory of the receiving plot is removed.
#' \item \code{pObjects$table_links} is modified if \code{input[[by_field]]} is equal to \code{title} (i.e., a linked table is being used for feature names)
#' to update the identity of the linked table based on the new table in \code{input[[tab_field]]}.
#' Conversely, if \code{input[[by_field]]} is not equal to \code{title}, the current panel is removed from all \code{pObjects$table_links}.
#' \item Link panel counters in \code{rObjects} are incremented if the new linked table differs from the old link table.
#' Counters are only updated for the current panel as well as the old/new tables.
                                  ...
                                  feat_choices, param='color') 
{
    ...
    if (old_tab!=tab) {
        # Updating the link UI.
        tab_names <- .decoded2encoded(setdiff(c(old_tab, tab), .noSelection))
