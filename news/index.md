...
- Fix bug in the reporting of collapsible box status. Addresses
  [\#701](https://github.com/iSEE/iSEE/issues/701)
...
- Add a parameter to control the title displayed in the browser tab,
  currently defaulting to “iSEE”. Thanks to
  [@RiboRings](https://github.com/RiboRings) for the implementation!
...
- Change the processing of the title for the app - if provided
  explicitly, now it can also correctly handle HTML content tags and
  more. Thanks to [@RiboRings](https://github.com/RiboRings) for
  spotting this, resolving
  [\#681](https://github.com/iSEE/iSEE/issues/681)

## iSEE 2.17.4

- Add explicitly a “Stop app” button to close the application (should be
  a means to nicely behave in container-spawned instances). Addresses
  [\#630](https://github.com/iSEE/iSEE/issues/630)
- Fix text in ‘configure’ vignette. Closes
  [\#626](https://github.com/iSEE/iSEE/issues/626)
- Copy information from rowRanges to rowData Closes
  [\#637](https://github.com/iSEE/iSEE/issues/637)
...
- If heatmap matrix has no finite values, set the color range
  arbitrarily. Closes [\#610](https://github.com/iSEE/iSEE/issues/610)
- Limit the number of rows in heatmap annotation legends to 10. Closes
  [\#591](https://github.com/iSEE/iSEE/issues/591)
- Fix license. Closes [\#661](https://github.com/iSEE/iSEE/issues/661).
- Rename some internal constants.
- Add checkbox to fix aspect ratio to 1. Closes
  [\#541](https://github.com/iSEE/iSEE/issues/541).
- Add checkbox to hide violin boundaries. Closes
  [\#619](https://github.com/iSEE/iSEE/issues/619).
...
- Fix bug introduced in `2.11.2` (DataBoxOpen would apply also to Visual
  parameters box)

## iSEE 2.13.1

- Define missing methods for generics for custom tables (issues
  [\#608](https://github.com/iSEE/iSEE/issues/608) and
  [\#612](https://github.com/iSEE/iSEE/issues/612))
...
- Use standard syntax to include empty icon in relevant places (issue
  [\#606](https://github.com/iSEE/iSEE/issues/606)).

## iSEE 2.11.2

- Make it possible to hide the “Visual Parameters” box (issue
  [\#611](https://github.com/iSEE/iSEE/issues/611)).

## iSEE 2.11.1

- Add `tooltip.signif` to
  [`registerAppOptions()`](https://isee.github.io/iSEE/reference/registerAppOptions.md)
  to regulate the number of significant digits shown in the tooltip.
...
- Enable customisation of hovering tooltip in `DotPlot` panels including
...
- Bugfix related to <https://github.com/rstudio/shiny/issues/3125>,
  ...
  [`DT::datatable()`](https://rdrr.io/pkg/DT/man/datatable.html) prior
  to [`selectInput()`](https://rdrr.io/pkg/shiny/man/selectInput.html).
...
- Partial bugfix avoiding re-rendering of `ComplexHeatmapPlot` panel
  when an incoming row selection changes if custom rows are in use. The
  partial bugfix only applies if the `ComplexHeatmapPlot` also disables
...
- Bugfix for conversion of categorical columns with too many levels to
  numeric ones
...
- Added functionality to use multiple row/column selections as a factor
  on the axes, for faceting or for coloring.
- Moved selection transparency setter into the “Visual parameters” box.
- Deprecated `SelectionEffect="Color"` in favor of
  `ColorBy="Column selection"` and `ColorBy="Row selection"`.
- Deprecated `SelectionColor` as the coloring for selections is
  determined using
  [`colDataColorMap()`](https://isee.github.io/iSEE/reference/ExperimentColorMap-class.md)
  instead.
...
- Added the
  ...
  generic to ensure all names in the SummarizedExperiment are present
  and unique.
...
- Disable the import button on the dimnames modal when transmitter has
...
- Enable custom saving of the application state via the new `saveState=`
  argument.
...
- [`.refineParameters()`](https://isee.github.io/iSEE/reference/setup-generics.md)
  for `FeatureAssayPlot`, `SampleAssayPlot` protects the x-axis choice
  against absent metadata.
- CSS classes for each panel are now defined at app run-time, to make it
  easier to write landing pages without specifying `initial=` in
  [`iSEE()`](https://isee.github.io/iSEE/reference/iSEE.md).

## iSEE 2.1.16

- Bugfix for initialization of the `ColorByFeatureDynamicSource` UI
  element in `ColumnDotPlot`s.

## iSEE 2.1.15

- Allow labelling of the medoid for each level of a discrete variable in
  a scatter-type `DotPlot`.
...
- Allowed custom annotation about selected table row to to be displayed
  in `Table` panels.
...
- Added panel-specific tours via the
...
- Added `HiddenColumns` slot to hide columns in `ColumnDataTable`s and
  `RowDataTable`s.

## iSEE 2.1.10

- Avoided transmitting multiple selections for `Table`s when the number
  of columns change.
- Hid irrelevant UI elements for `Table` multiple selections.
- Generalized information about the number of selected rows/columns in
  each panel.
...
- Added control of legend point size under the “Text” category of the
...
- Fixed initialization of panel size to current value when the “Organize
  panels” window is closed and re-opened.
- Fixed removal of last panel from the interface.

## iSEE 2.1.4

- Added progress bar when exporting panel outputs.
- Fixed missing section in
...
- Added vignette documenting the use of out-of-memory matrices for big
  data.
- Added `TENxPBMCData` to `Suggests:`.

## iSEE 1.99.9

- Fixed slot names in the
  [`ReducedDimensionPlot()`](https://isee.github.io/iSEE/reference/ReducedDimensionPlot-class.md)
  man page.

## iSEE 1.99.8

- Protected against transient invalid selected index; fixes
  [\#400](https://github.com/iSEE/iSEE/issues/400).
...
- Fixed bug around `NA` groupings in
...
- Added
  ...
  methods to intercept choices of x/y-axis variables.

## iSEE 1.99.1

- Allowed customization of landing pages (from calling
  [`iSEE()`](https://isee.github.io/iSEE/reference/iSEE.md) in no-SE
  mode) for enterprise deployments.
- Allowed export of plot and table panel outputs as PDF and CSV files,
  respectively.
...
- Added
  ...
  to provide on-ramp for making customized panels.
- Allowed global setting using `iSEEOptions`.
- Expanded class and slot names from “Feat” to “Feature”, “Samp” to
  “Sample”, “RedDimPlot” to “ReducedDimensionPlot”.
...
- Fix [`rbind()`](https://rdrr.io/pkg/BiocGenerics/man/cbind.html) of
  `data.frame` and `DataFrame` objects.
- Fix error related to using `&&` with variable length greater than 1.
- Replace deprecated `scater` argument.
- Replace deprecated functions:
  [`SingleCellExperiment::clearSpikes()`](https://rdrr.io/pkg/SingleCellExperiment/man/defunct.html),
  [`SingleCellExperiment::clearSizeFactors()`](https://rdrr.io/pkg/SingleCellExperiment/man/defunct.html).
...
- Support gene list input from `aceEditor()` and
  [`fileInput()`](https://rdrr.io/pkg/shiny/man/fileInput.html).

## iSEE 1.5.11

- Renamed `isColorMapCompatible()` to
  [`checkColormapCompatibility()`](https://isee.github.io/iSEE/reference/checkColormapCompatibility.md).
- Fixed graceful server side handling of
  [`checkColormapCompatibility()`](https://isee.github.io/iSEE/reference/checkColormapCompatibility.md).
...
- Substituted deprecated `scater::normalize()` by
  [`scater::logNormCounts()`](https://rdrr.io/pkg/scuttle/man/logNormCounts.html).

## iSEE 1.5.7

- Simplified protection of `redDimPlotDefaults()` against empty
  `reducedDims`.
...
- Updated calls to
  ...
  to load only `tophat_counts` assay.
...
- Moved panel organization to modal with selectize to control panel
...
- Fixed initialization of search fields for tables that are initialized
...
- Added missing observer for colorpicker when colouring by feature name
  in row-based plots, or by sample name in column-based plots.
- Ignore `NA` values when computing the range of coloring scales.
- Added a size expansion factor (5x) to the selected point when
  colouring by feature name in row-based plots, or by sample name in
  column-based plots.
- Fixed redundant coloring of selected point when colouring by feature
...
- Fixed colour scale to be invariant when selecting on a different
  color.
...
- Fixed initialization of reduced dimensions with a single plot axis
  choice.
- Substituted discouraged use of
...
- Enabled faceting by row and column, with appropriate updates to brush
  and lasso.
- Enabled shaping on data points.
- Minor fix of jitter for violin and square plots.
- INTERNAL: Enabled storage of additional `plot.data` beyond `X` and `Y`
  in `all.coordinates`. See constant `.allCoordinatesNames`. Necessary
  for correct behaviour of brushes on faceted plots.
...
- Renamed feature expression plots to feature assay plots, for
  generality.
