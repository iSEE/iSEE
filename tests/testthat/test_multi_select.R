...
memory <- list(
    ...
    RowDataPlot(SelectRowSource="SampAssayPlot1"),
    SampAssayPlot(),
    RowStatTable()
)
...
test_that(".process_selectby_choice works with a column-based brush", {
    ...
    pObjects$memory$RedDimPlot1[[iSEE:::.brushData]] <- list()
})
...
test_that(".process_selectby_choice works with saved column selections", {
    pObjects$memory$RedDimPlot1[[iSEE:::.multiSelectHistory]] <- list(DUMMY_BRUSH)
...
test_that(".process_selectby_choice works with saved row selections", {
    pObjects$memory$SampAssayPlot1[[iSEE:::.multiSelectHistory]] <- list(DUMMY_BRUSH)
