...
test_that(".process_selectby_choice works with a column-based brush", {
    ...
    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$brushData]] <- DUMMY_BRUSH
    plot_env <- new.env()
    cmds <- .processMultiSelections(pObjects$memory$ColumnDataPlot1, pObjects$memory, pObjects$contents, plot_env)
    ...
    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$brushData]] <- list()
})
...
test_that(".process_selectby_choice works with a row-based brush", {
    ...
    pObjects$memory$SampleAssayPlot1[[iSEEslots$brushData]] <- DUMMY_BRUSH
    plot_env <- new.env()
    cmds <- .processMultiSelections(pObjects$memory$RowDataPlot1, pObjects$memory, pObjects$contents, plot_env)
    ...
    pObjects$memory$SampleAssayPlot1[[iSEEslots$brushData]] <- list()
})
...
test_that(".process_selectby_choice works with a column-based lasso", {
    plot_env <- new.env()
    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$brushData]] <- OPEN_LASSO
    cmds <- .processMultiSelections(pObjects$memory$ColumnDataPlot1, pObjects$memory, pObjects$contents, plot_env)
    expect_false(exists('col_selected', envir=plot_env))
    expect_identical(length(cmds), 0L)

    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$brushData]] <- DUMMY_LASSO
    plot_env <- new.env()
    cmds <- .processMultiSelections(pObjects$memory$ColumnDataPlot1, pObjects$memory, pObjects$contents, plot_env)
...
test_that(".process_selectby_choice works with a row-based lasso", {
    plot_env <- new.env()
    pObjects$memory$SampleAssayPlot1[[iSEEslots$brushData]] <- OPEN_LASSO
    cmds <- .processMultiSelections(pObjects$memory$RowDataPlot1, pObjects$memory, pObjects$contents, plot_env)
    expect_false(exists('row_selected', envir=plot_env))
    expect_identical(length(cmds), 0L)

    pObjects$memory$SampleAssayPlot1[[iSEEslots$brushData]] <- DUMMY_LASSO
    plot_env <- new.env()
    cmds <- .processMultiSelections(pObjects$memory$RowDataPlot1, pObjects$memory, pObjects$contents, plot_env)
...
test_that(".process_selectby_choice works with saved column selections", {
    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$multiSelectHistory]] <- list(DUMMY_BRUSH)
    ...
    pObjects$memory$ReducedDimensionPlot1[[iSEEslots$multiSelectHistory]] <- list()
})

test_that(".process_selectby_choice works with saved row selections", {
    pObjects$memory$SampleAssayPlot1[[iSEEslots$multiSelectHistory]] <- list(DUMMY_BRUSH)
    ...
    pObjects$memory$SampleAssayPlot1[[iSEEslots$multiSelectHistory]] <- list()
})
