...
test_that("selection link creation works correctly", {
    ...
    redDimArgs[1,.selectByPlot] <- "Feature assay plot 1"
    featAssayArgs[1,.selectByPlot] <- "Column data plot 2"
    colDataArgs[2,.selectByPlot] <- "Reduced dimension plot 1"

    expect_error(
        .spawn_selection_chart(list(
            redDimPlot=redDimArgs,
            colDataPlot=colDataArgs,
            featAssayPlot=featAssayArgs,
            rowStatTable=rowStatArgs,
    ...
    redDimArgs[1, .selectByPlot] <- "whee!"
    expect_error(
        .spawn_selection_chart(list(
            redDimPlot=redDimArgs,
            colDataPlot=colDataArgs,
...
test_that("selection link updates work correctly", {
    ...
    g2 <- .choose_new_selection_source(g, "redDimPlot1", "---", "featAssayPlot1")
    ...
    g2 <- .choose_new_selection_source(g, "redDimPlot1", "---", "colDataPlot1")
    expect_equal(g[], g2[])
    ...
    expect_equal(sum(g[]), 3)
    g2 <- .choose_new_selection_source(g, "colDataPlot1", "featAssayPlot3", "---")
    ...
    expect_equal(sum(g[]), 3)
    g2 <- .choose_new_selection_source(g, "colDataPlot2", "featAssayPlot2", "redDimPlot1")
    ...
    g2 <- .choose_new_selection_source(g, "redDimPlot1", "featAssayPlot1", "featAssayPlot1")
    expect_equal(g[], g2[])
})
...
test_that("selection source destruction works correctly", {
    ...
    expect_identical("Feature assay plot 1", pObjects$memory$redDimPlot[1,.selectByPlot])
    expect_identical("Feature assay plot 1", pObjects$memory$featAssayPlot[1,.selectByPlot])

    .destroy_selection_panel(pObjects, "featAssayPlot1")
    ...
    expect_identical("---", pObjects$memory$redDimPlot[1,.selectByPlot])
    expect_identical("---", pObjects$memory$featAssayPlot[1,.selectByPlot])
    ...
    expect_equal(sum(pObjects$selection_links[]), 3)
    expect_identical("Feature assay plot 1", pObjects$memory$redDimPlot[1,.selectByPlot])
    expect_identical("Reduced dimension plot 1", pObjects$memory$colDataPlot[2,.selectByPlot])

    .destroy_selection_panel(pObjects, "redDimPlot1")

    expect_false(igraph::are_adjacent(pObjects$selection_links, "featAssayPlot1", "redDimPlot1"))
    expect_false(igraph::are_adjacent(pObjects$selection_links, "redDimPlot1", "colDataPlot2"))
    expect_equal(sum(pObjects$selection_links[]), 1)
    expect_identical("---", pObjects$memory$redDimPlot[1,.selectByPlot])
    expect_identical("---", pObjects$memory$colDataPlot[2,.selectByPlot])
    ...
    .destroy_selection_panel(pObjects, "colDataPlot2")
...
test_that("select dependent identification works correctly", {
    ...
    featAssayArgs[1,.selectByPlot] <- "Reduced dimension plot 1"
    featAssayArgs[2,.selectByPlot] <- "Reduced dimension plot 1"
    colDataArgs[1,.selectByPlot] <- "Feature assay plot 1"
    colDataArgs[2,.selectByPlot] <- "Feature assay plot 2"
    featAssayArgs[3,.selectByPlot] <- "Column data plot 1"
    ...
    memory <- list(
        ...
        heatMapPlot=heatMapArgs)
    g <- .spawn_selection_chart(memory)
    expect_identical(.get_selection_dependents(g, "redDimPlot1", memory),
        c("featAssayPlot1", "featAssayPlot2"))

    # Restriction in one of the children, and not the other.
    featAssayArgs[1,.selectEffect] <- .selectRestrictTitle
    memory <- list(
        redDimPlot=redDimArgs,
        colDataPlot=colDataArgs,
        featAssayPlot=featAssayArgs,
    ...
    colDataArgs[1,.selectEffect] <- .selectRestrictTitle
    memory <- list(
        redDimPlot=redDimArgs,
        colDataPlot=colDataArgs,
        featAssayPlot=featAssayArgs,
    ...
    featAssayArgs[1,.selectEffect] <- .selectColorTitle
    memory <- list(
        redDimPlot=redDimArgs,
        colDataPlot=colDataArgs,
        featAssayPlot=featAssayArgs,
...
test_that("brush identity function works properly", {
    expect_true(.identical_brushes(list(xmin=1, xmax=2, ymin=10, ymax=20),
        list(xmin=1, xmax=2, ymin=10, ymax=20)))

    # Confirming correct failure.
    expect_false(.identical_brushes(list(xmin=1, xmax=2, ymin=10, ymax=20),
...
test_that("evaluation order works properly", {
    ...
    eval_order <- .establish_eval_order(g)
...
test_that("selections involving custom panels work correctly", {
    ...
    customDataArgs[1,.customColSource] <- "Reduced dimension plot 1"
    customDataArgs[1,.customRowSource] <- "Row data plot 1"
    customStatArgs[1,.customColSource] <- "Reduced dimension plot 2"
    customStatArgs[1,.customRowSource] <- "Sample assay plot 2"
    ...
    expect_identical(sort(.get_selection_dependents(g, "redDimPlot1", memory)), c("colDataPlot2", "customDataPlot1"))
    expect_identical(.get_selection_dependents(g, "customDataPlot1", memory), character(0))
    expect_identical(sort(.get_selection_dependents(g, "sampAssayPlot2", memory)), "customStatTable1")
    expect_identical(.get_selection_dependents(g, "customStatTable1", memory), character(0))
    ...
    expect_identical(pObjects$memory$customDataPlot[[1, .customColSource]], .noSelection)

    .destroy_selection_panel(pObjects, "sampAssayPlot2")
    expect_identical(sort(names(igraph::neighbors(pObjects$selection_links, "customStatTable1", mode="in"))), "redDimPlot2")
    expect_identical(pObjects$memory$customStatTable[[1, .customRowSource]], .noSelection)
})
