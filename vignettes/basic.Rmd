...
`r Biocpkg("iSEE")` is a [Bioconductor](http://bioconductor.org) package that provides an interactive Shiny-based graphical user interface for exploring data stored in objects derived from the `SummarizedExperiment` class, which includes `SingleCellExperiment` objects [@kra2018iSEE].

Instructions to install the package are available `r Biocpkg("iSEE", label = "here")`. Once installed, the package can be loaded as follows:

```{r library}
library(iSEE)
```

If you have already loaded a `SummarizedExperiment` or `SingleCellExperiment` object into your workspace named `se`, you can launch an `r Biocpkg("iSEE")` app by running:

```{r quickstart, eval=FALSE}
iSEE(se)
```

In this vignette, we demonstrate this process using the `allen` single-cell RNA-seq data set from the `r Biocpkg("scRNAseq")` package. To try out the app using this example dataset, run the following code.
...
The `allen` data set contains expression values for 379 cells from the mouse visual cortex [@tasic2016adult]. To begin with, we load and inspect the `allen` object.
...
As provided, the `allen` object contains raw data and a number of quality control and experimental cell annotations, all available in `colData(allen)`.

```{r colData_sce}
colnames(colData(allen))
```

Then, we convert the provided `SummarizedExperiment` object to a `SingleCellExperiment` object and normalize the expression values with `r Biocpkg("scater")`. 
