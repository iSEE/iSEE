...
Instructions to install the package are available `Biocpkg("iSEE", label = "here")`.
...
As provided, the `SummarizedExperiment` object only contains raw data and not much else of interesting information.
Thus, we need to fill it up for some more exciting visualization.
To begin with, we load and inspect the `allen` object.
...
Then, we start by converting the provided `SummarizedExperiment` object to a `SingleCellExperiment` object and normalizing the expression values with `r Biocpkg("scater")`. 
...
We will also add some gene metadata to the `rowData` related to the mean-variance relationship in the data.
...
Clicking these buttons, additional panels of the corresponding type can be inserted into the main body of the app.
