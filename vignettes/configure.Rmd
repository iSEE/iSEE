...
```{r fexArg}
app <- iSEE(sce, initial=list(
    RowAssayPlot(YAxisFeatName="0610009L18Rik"),
    RowAssayPlot(YAxisFeatName="0610009B22Rik")
)) 
```
...
```{r xaxis}
fex <- RowAssayPlot(DataBoxOpen=TRUE, PanelWidth=6L)
...
fex4[["XAxisRowTable"]] <- "RowDataTable1"
...
app <- iSEE(sce, initial=list(
    ReducedDimPlot(DataBoxOpen=TRUE, Type="TSNE", 
...
app <- iSEE(sce, initial=list(
    RowAssayPlot(DataBoxOpen=TRUE, Assay="logcounts", PanelWidth=6L),
    RowAssayPlot(DataBoxOpen=TRUE, Assay="tophat_counts", PanelWidth=6L)
))
```
...
```{r facet}
cdp <- ColumnDataPlot(VisualBoxOpen=TRUE, VisualChoices="Facets",
...
cdArgs <- ColumnDataPlot(SelectBoxOpen=TRUE, SelectColSource="ReducedDimPlot1",
