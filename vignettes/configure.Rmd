...
```{r fexArg}
app <- iSEE(sce, initial=list(
    FeatureAssayPlot(YAxisFeatName="0610009L18Rik"),
    FeatureAssayPlot(YAxisFeatName="0610009B22Rik")
)) 
```
...
```{r yaxis}
app <- iSEE(sce, initial=list(
    SampleDataPlot(YAxis="NREADS", PanelWidth=6L, DataBoxOpen=TRUE),
    SampleDataPlot(YAxis="TOTAL_DUP", PanelWidth=6L, DataBoxOpen=TRUE)
))
```
...
cdp <- SampleDataPlot(VisualBoxOpen=TRUE, 
...
cdp <- SampleDataPlot(VisualBoxOpen=TRUE, VisualChoices=c("Color", "Shape"),
...
```{r facet}
cdp <- SampleDataPlot(VisualBoxOpen=TRUE, VisualChoices="Facets",
...
cdArgs <- SampleDataPlot(SelectBoxOpen=TRUE, SelectColSource="ReducedDimensionPlot1",
