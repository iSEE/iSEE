...
```{r fexArg}
app <- iSEE(sce, initial=list(
    FeatAssayPlot(YAxisFeatName="0610009L18Rik"),
    FeatAssayPlot(YAxisFeatName="0610009B22Rik")
)) 
```
...
```{r yaxis}
app <- iSEE(sce, initial=list(
    ColDataPlot(YAxis="NREADS", PanelWidth=6L, DataBoxOpen=TRUE),
    ColDataPlot(YAxis="TOTAL_DUP", PanelWidth=6L, DataBoxOpen=TRUE)
))
```
...
```{r xaxis}
fex <- FeatAssayPlot(DataBoxOpen=TRUE, PanelWidth=6L)
...
fex2[["XAxisColData"]] <- "Core.Type"

# Example 3a
fex3 <- fex
fex3[["XAxis"]] <- "Feature name"
fex3[["XAxisFeatName"]] <- "Zyx"

# Example 4 (also requires a row statistic table)
fex4 <- fex
fex4[["XAxis"]] <- "Feature name"
fex4[["XAxisRowTable"]] <- "RowStatTable1"
rex <- RowStatTable(Selected="Ints2", Search="Ints", PanelWidth=12L)
...
app <- iSEE(sce, initial=list(
    RedDimPlot(DataBoxOpen=TRUE, Type="TSNE", 
...
app <- iSEE(sce, initial=list(
    FeatAssayPlot(DataBoxOpen=TRUE, Assay="logcounts", PanelWidth=6L),
    FeatAssayPlot(DataBoxOpen=TRUE, Assay="tophat_counts", PanelWidth=6L)
))
```
...
cdp <- ColDataPlot(VisualBoxOpen=TRUE, 
    VisualChoices=c("Color", "Points", "Other"))
...
cdp <- ColDataPlot(VisualBoxOpen=TRUE, VisualChoices=c("Color", "Shape"),
    ColorByColData="Core.Type", ShapeByColData="Core.Type",
    ColorBy="Column data", ShapeBy="Column data")

cdp2 <- cdp
cdp2[["ColorByColData"]] <- "TOTAL_DUP"
cdp2[["ShapeByColData"]] <- "driver_1_s"
...
```{r facet}
cdp <- ColDataPlot(VisualBoxOpen=TRUE, VisualChoices="Facets",
...
cdArgs <- ColDataPlot(SelectBoxOpen=TRUE, SelectColSource="RedDimPlot1",
    SelectEffect="Color", SelectColor="purple", 
