...
- `"Column data"`: requires `"XAxisColData"` to be set to a column name in the `colData(sce)`
- `"Feature name"`: requires either
    - `"XAxisFeatName"` to be set to a feature name (or positional index) in `rownames(sce)`
    - `"XAxisRowTable"` and `"XAxisFeatName"` to be both set, to the name of a _Row statistics table_ panel and the name (or index) of a feature in that table
...
Note how _Example 4_ requires an active _row statistics table_.
