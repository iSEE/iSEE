...
To demonstrate _custom plot panels_, we define an example function `CUSTOM_PCA` below.
...
To demonstrate _custom table panels_, we define an example function `CUSTOM_SUMMARY` below.
This takes a subset of features and cells in a `SingleCellExperiment` object,
...
CUSTOM_SUMMARY <- function(se, ri, ci, assay="logcounts", min_exprs=0) {
    ...
    data.frame(
        Mean = rowMeans(assay(se, assay)[ri, ci]),
        Var = rowVars(assay(se, assay)[ri, ci]),
        Sum = rowSums(assay(se, assay)[ri, ci]),
        n_detected = rowSums(assay(se, assay)[ri, ci] > min_exprs),
        row.names = ri
    )
}
```
...
Each line corresponds to one space-separated argument:value pair, and the name of the argument is defined as the string before the first space.
