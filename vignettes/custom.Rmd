...
app <- iSEE(sce,
    initial=list(
        ColDataPlot(XAxis="Column data", XAxisColData="Primary.Type", 
            PanelId=1L),
        GENERATOR(mode="TSNE", ntop=1000, SelectColSource="ColDataPlot1")
    )
)
```
...
app <- iSEE(sce,
    initial=list(
        ...
        GENERATOR(SelectColSource="RedDimPlot1")
    )
)
```
...
app <- iSEE(sce, initial=list(RedDimPlot(PanelId=1L),
    TAB_GEN(SelectColSource="RedDimPlot1"),
    HEAT_GEN(SelectColSource="RedDimPlot1"))
)
```
