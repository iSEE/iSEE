...
`r Biocpkg("iSEE")` coordinates the coloration in every plot via the `ExperimentColorMap` class.
...
It is the user's responsibility to ensure that all levels are accounted for.
...
# Explaining the colormap hierarchy
...
We will again use the `allen` single-cell RNA-seq data set to demonstrate the use of the `ExperimentColorMap` class.
First, we set up the `SingleCellExperiment` object containing all of the necessary results:
...
We create an `iSEE` app with the `SingleCellExperiment` object and the colormap generated above.
