...
Probably the most immediate way to master the user interface of `r Biocpkg("iSEE")` is by taking the guided interactive tour, as this feature provides a pretty exhaustive showcase of the relevant elements and possible user interactions. Nonetheless, a brief description of the UI (based on the `shinydashboard` package) is provided in this section.
...
Typically, these tables can be used to handily connect to other plots, but also to retrieve on the fly (via the `rentrez` package, as well as linking to the NCBI database) some information on the selected gene, provided the `annot.orgdb`, `annot.keytype`, and `annot.keyfield` parameters are specified.
