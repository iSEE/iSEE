...
However, they can also retrieve gene-specific annotation on the fly by specifying the `annotFun` parameter, e.g. by using the `annotateEntrez` or `annotateEnsembl` functions, provided in the package. Alternatively, users can create a customized annotation function; for more details on this, please consult the manual pages of `annotateEntrez` or `annotateEnsembl`.
