...
The generated plots can be conveniently linked to each other by brushing (i.e. clicking and holding the mouse over a plot area), and with a nice touch of reproducibility, you can obtain the full code to re-create the plots at any point during your live session. The combination of interactivity and reproducibility follows the proposal in the `r Biocpkg("pcaExplorer")` and `r Biocpkg("ideal")` packages, to deliver an immediate user experience, and guarantee the aspect of technical reproducibility of the generated output.
...
- once you have started the app, look in the upper right corner for a question mark icon. Click on that, and be ready to perform an interactive tour of the app, based on the `r CRANpkg("rintrojs")` package. []ganz[] During this tour, you will be taken through the different components of `r Biocpkg("iSEE")` and learn the basic usage mechanisms by doing: the highlighted elements will be responding to the user's actions, while the rest of the UI will be shaded. You can move forward and backward along the tour by clicking on the `Next`/`Back` buttons, or also using the arrow keys. To exit the tour, either click on `Skip`, or simply click outside of the highlighted UI element.  

Once you are done generating the plots, you can export the code that generated the outputs, by clicking on the sidebar button with the magic wand icon. This will open a modal popup where the code is displayed and automatically highlighted in a `r CRANpkg("ShinyAce")`-based text editor. Select it all, or parts of it, copy it to the clipboard, and paste it into your analysis script/Rmarkdown file.


# Using the application

This section explains more in detail the different parameters you can adjust when launching the `iSEE` app. This set refers to a local instance of `r Biocpkg("iSEE")`.
...
Probably the most immediate way to master the user interface of `r Biocpkg("iSEE")` is by taking the guided interactive tour, as this feature provides a pretty exhaustive showcase of the relevant elements and possible user interactions. Nonetheless, a brief description of the UI (based on the `r CRANpkg("shinydashboard")` package) is provided in this section.

## The `r Biocpkg("iSEE")` header

The dashboard header for `r Biocpkg("iSEE")` contains, apart from the title, a dropdown menu, where a number of help elements are available:
...
- The “Open the vignette (web)" button opens in a new tab the vignette for `r Biocpkg("iSEE")` from the Bioconductor website (internet connection is required). Please keep in mind it will be referring to the most current release version (the documentation might differ from the version installed on your system)
...
- “About iSEE" displays a modal popup containing information about how to cite `r Biocpkg("iSEE")`, as well as the output from `sessionInfo()`, which should be included in case of reporting a bug/issue


## The `r Biocpkg("iSEE")` sidebar

In the dashboard sidebar of `r Biocpkg("iSEE")`, 4 buttons allow the creation of new plot elements in the main body of the app. The maximum number of plots per type can be defined when launching the app. Upon clicking either of these buttons, additional plots or tables are inserted, together with placeholders to reorganize the sequence of how they should appear. This can be controlled by means of the up and down arrow buttons, and unnecessary plots can be removed by clicking on the trash button. Also, the width of each panel can be adjusted, in a value ranging from 2 to 12 (12 being the width of a full `fluidRow`).

To help the user in organizing the plots, a color coding reminds the plot type being moved/canceled []TODO[]

A fundamental element in the sidebar is the `Extract the R code!` button (with the magic wand icon). By clicking on this, the user opens up a modal popup window, where an instance of a text editor (based on the `r CRANpkg("ShinyAce")` package) contains the code snippet required to reproduce the entire plots, created in the live session. This is typically used at the end of each exploration, to extract a reproducible snapshot of the commands, which can be furthermore edited, in order to finalize the plots (e.g. to get ready for publication). 



## The `r Biocpkg("iSEE")` dashboard body

The main element in the body of `r Biocpkg("iSEE")` is the combination of plots and tables, generated (and optionally linked to one another) according to the different actions taken by the user. The main explanation on how the different plots and tables work is presented in Section \@ref(functionality).

At the bottom of the body, a footer including some information on the `r Biocpkg("iSEE")` development team can be found.



# Functionality {#functionality}

This section contains a description of each plot and table type you can generate with `r Biocpkg("iSEE")`. For each plot panel, three different sets of parameters will be available in collapsible boxes, namely `Plotting parameters`, `Coloring parameters`, and `Brushing parameters`. While the plotting parameters will be specific to each plot type, the coloring and brushing panels look the same for all types. How the brushing operates is described in additional detail in Section \@ref(brushing).

## Reduced dimension plots

[]screenshot plot + plot params[]
You can obtain a reduced dimension plot (e.g. PCA, t-SNE) of your expression dataset to efficiently explore the high-dimensional dataset under inspection. The plotting parameters control the `redDim` slot to be displayed, as well as the two dimensions to plot against each other. Note that `iSEE` does not calculate the reduced dimension representations, so they must be present in the provided object.

You can color your points (i.e. cells or samples) in different ways. Default is no color scheme (`None` in the radio button), but it is possible to use the value of any `Column data`, be it discrete or continuous. The plotting routine recognizes this, and adapts automatically the scale to use. Another possibility is to color by expression value of a particular gene, either via the linked `Gene table`, or via the `Gene text` which is input in the text field (this needs to be one of the row names in the `SingleCellExperiment`/`SummarizedExperiment` object). The selection via gene table is made possible by clicking on one row in the corresponding `datatable` element. Among the different choices for expression values, it is possible to select one of the `assays` in the provided `SingleCellExperiment`/`SummarizedExperiment`.  
...
A gene statistics table contains by default the values of the `rowData` slot for the `SingleCellExperiment`/`SummarizedExperiment` object. If none are specified, a column named `Present` is added and set to `TRUE` for all available genes, in order to guarantee the functionality of a `datatable`.

Typically, these tables can be used to handily connect to other plots, but also to retrieve on the fly (via the `r CRANpkg("rentrez")` package, as well as linking to the NCBI database (https://www.ncbi.nlm.nih.gov/)) some information on the selected gene, provided the `annot.orgdb`, `annot.keytype`, and `annot.keyfield` parameters are specified.
...
- `Color` will mark the set of points in the destination plot with a color that can be selected via the `r CRANpkg("colourpicker")` package 
...
In this section, we illustrate how `r Biocpkg("iSEE")` can be used to explore the `allen` single-cell RNA-seq data set, which is available via the `r Biocpkg("scRNAseq")` Bioconductor package and contains expression values for 379 cells from the mouse visual cortex [@Tasic2016]. We start by converting the provided `SummarizedExperiment` object to a `SingleCellExperiment` object and normalize the expression values. 
...
Next, we apply Principal Components Analysis (PCA) and t-distributed Stochastic Neighbour Embedding (t-SNE) to generate two reduced dimension representations of the cells. The provided cell annotations for this data set are available in `colData(sce)`. Note that all calculations (dimension reduction, clustering, etc) must be performed *before* passing the object to `iSEE`.

```{r allen-dataset-2,eval=FALSE}
sce <- runPCA(sce)
sce <- runTSNE(sce)
head(colData(sce))
```

Finally, we set colour palettes to use in `r Biocpkg("iSEE")` when colouring according to specific cell annotations or expression values. These values are stored in an object of the `ExperimentColorMap` class. 

```{r allen-dataset-3,eval=FALSE}
...
To begin the exploration, we launch the `iSEE` app with the `SingleCellExperiment` object and the colormap generated above.

```{r allen-dataset-4,eval=FALSE}
iSEE(sce, colormap = ecm)
```

By default, the app starts with a dashboard containing one reduced dimension plot, one column data plot, one gene expression plot and one gene statistics table (see Use case II for how to change this). By opening the collapsible panels named “Plotting parameters", “Coloring parameters" and “Brushing parameters" under each plot we can see and change what is being displayed in the respective plots, and also how it is being displayed. 
...
The default start configuration with one plot of each type is not optimal for all use cases, but the flexibility of `iSEE` allows the user to easily modify it. Doing this programmatically can also save time since in this way, the app can be started in any configuration of the user’s choice, removing the need to click through the choices to get there interactively. Let’s say, for example, that we are only interested in gene expression plots, but that we would like multiple of these, showing a preselected set of genes. The default set of panels can be changed via the `initialPanels` argument to `iSEE()`. Given a `SingleCellExperiment`/`SummarizedExperiment` `sce`, the following code opens an app with two adjacent gene expression plots. 

```
iSEE(sce, 
     initialPanels = DataFrame(Name = c("Gene expression plot 1", "Gene expression plot 2"),
                               Width = c(6, 6))
)
```

The genes to show on the y-axis in the two plots can be specified via the `geneExprArgs` argument to `iSEE`. This is a `DataFrame`, specifying the initial parameters for each plot. In this case, we want to modify the `YAxis` and `YAxisGeneText` defaults for the two plots (for a full list of values that can be specified see `?defaults`):

```
gexArg <- DataFrame(YAxis = "Gene text", YAxisGeneText = c("0610009L18Rik", "0610009B22Rik"))
...
This will open the app with two gene expression plots, showing the selected genes. Of course, from this starting point, all the interactive functionality is still available, and new panels can be added, modified and linked via brushing. 
...
## Use case IV: Using the `ExperimentHub`

Here we will use `r Biocpkg("iSEE")` showcasing TCGA dataset
