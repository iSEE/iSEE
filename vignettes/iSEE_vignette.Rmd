...
In the absence of any user-defined colormap, the application falls back to a set of two default colormaps -- a categorical and a continuous -- according to the nature of the data displayed.
...
Conversely, categorical colormap functions may ignore their argument, as the number of factor levels is typically known in advance, and impose a fixed number of corresponding colors.
Furthermore, categorical colormaps may typically return a named vector of colors, with the name declaring which factor level each color is assigned to.
...
- *globally shared* colormaps can be defined for all categorical or continuous data that do not have colormaps defined in the previous two levels.
...
- if it is not defined, the *shared* colormap of the appropriate slot is looked up, according to the categorical or continuous nature of the data (for instance, `@all_continuous$colData` is looked up for continous column data without a defined individual colormap)
- if it is not defined, the *globally shared* colormap is looked up, according to the categorical or continuous nature of the data (for instance, `@global_continuous` is looked up for continous column data without a defined individual colormap, in the absance of a *shared* colData continuous colormap)
- lastly, if none of the above colormaps were defined, the `ExperimentColorMap` will return the default colormap, according to the categorical or continuous nature of the data.
...
Furthermore, default colormap for all variables can also be altered for continuous and categorical variable, respectively.
...
By default, the app starts with a dashboard that contains one reduced dimension plot, one column data plot, one feature assay plot, one row statistics table, one row data plot (if data is available), and one heat map (see Use case II for how to change this).
...
Given a `SingleCellExperiment`/`SummarizedExperiment` named `sce`, the following code opens an app with two adjacent feature assay plots. 
