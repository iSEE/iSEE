...
```{r, eval=FALSE}
gexArg <- DataFrame(YAxis = "Gene text", 
...
The `SummarizedExperiment` object also contains a t-SNE representation of the 5,000 cells and the results of a cell clustering performed with `r Biocpkg("FlowSOM")` [@vangassen2015flowsom] in order to automatically detect cell populations. 
...
The code below downloads the processed data file and sets up the initial panel configuration and brushing relationships. 

```{r, eval=FALSE}
cytof <- readRDS(gzcon(url("XXX.rds")))
```
