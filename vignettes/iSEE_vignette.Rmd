...
To link one plot to another, users can instruct a plotting panel to receive a selection of data point from another (transmitting) plot, using the appropriate field in the brushing parameters box.
...
Let us start by exploring the reduced dimension plot. As can be seen from the "Data parameters" panel, this plot shows the first two principal components.
...
Now let’s move on to the column data plot. Here, we see the distribution of the number of reads (`NREADS`) across the cells in the data set, as well as the individual values for each cell.
...
After exploring the individual plots, we will now see how they can be linked together using the point selection functionality.
...
initialPanels = DataFrame(Name = c(paste("Feature expression plot", 1:5), 
                                   "Reduced dimension plot 1"),
                          Width = c(rep(3, 5), 5))
fexArg <- DataFrame(XAxis = "Gene text", YAxis = "Gene text",
                    XAxisGeneText = c("Cell_length", "CD3", "CD7", "CD3", "CD4"), 
                    YAxisGeneText = c("CD45", "CD20", "CD45", "CD33", "CD45"),
                    BrushByPlot = c("", paste("Feature expression plot", 1:4)),
                    BrushEffect = "Restrict")
redArg <- DataFrame(ColorBy = "Column data", 
                    ColorByColData = "population", 
                    BrushByPlot = "Feature expression plot 5",
                    BrushEffect = "Restrict")
```
