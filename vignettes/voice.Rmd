---
title: Controlling the `iSEE` interface by voice
author:
...
output: 
  BiocStyle::html_document:
    toc_float: true
vignette: >
  %\VignetteIndexEntry{5. Voice control}
...
To keep the vocal commands reasonably short, only one panel may be under vocal control at any one time.
All vocal commands will affect the currently active panel, until a new panel is selected for vocal control.
See section [Vocal commands available](#availableVocalCommands).

# Implementation

We use the [_annyang_](https://github.com/TalAter/annyang) JavaScript library to handle speech recongnition and update _Shiny_ reactive values in the same way as mouse and keyboard UI elements trigger panel updates.
...
With `voice=TRUE`, the tiny JavaScript speech recognition library _annyang_ is loaded and activated in any web browser window that runs `app`.
...
As a proof-of-concept, we have currently implemented only a subset of vocal commands, compare to all the interactions possible using the mouse and keyboard.
...
* "**good boy**": If the app is behaving well, throw it a bone!
